...
* [x] 将CPU代码合并到一个`.v`文件, 文件名为`ysyx_学号后六位.v`, 如`ysyx_210888.v`
 * 在Linux上可通过`cat`命令实现:
 ```bash
 cat CPU.v ALU.v regs.v ... > ysyx_210888.v
 ```
* [x] 将CPU顶层命名修改为`ysyx_学号后六位.v`, 如`ysyx_210888`
* [x] 按照[CPU接口命名规范](./ysyx/soc/cpu-interface.md)修改CPU顶层端口名
* [x] 为CPU内的所有模块名添加前缀`ysyx_学号后六位_`
...
* [x] 运行[命名规范自查脚本](./ysyx/soc/cpu-check.py),
  运行方法参考[这里](./ysyx/soc/cpu-interface.md#命名规范自查脚本使用说明)

### CPU内部修改(2021/10/07 23:59:59前完成)

* [x] 所有触发器都需要带复位端, 使其复位后带初值
* 若实现了cache, 则需要
 * [x] 确认ICache和DCache的data array的大小均不大于4KB
 * [x] 确认ICache和DCache的data array均采用单口RAM
 * [x] 对data array进行RAM替换: 我们提供接口与流片用RAM一致的简化行为模型,
...
* 若采用Verilog开发, 则需要
 * [x] 确认代码中的锁存器(Latch)已经去除
    * Chisel福利: Chisel不会生成锁存器
 * [x] 确认代码中的异步复位触发器已经去除, 或已经实现同步撤离
...
* [x] 对代码进行规范检查, 清除报告的Warning. 具体步骤请参考[这里](./ysyx/lint/README.md)

### Verilator仿真(2021/11/01 23:59:59前完成)

* [x] 确认清除Warning后的代码可以成功启动RT-Thread
* [x] 将CPU集成到本项目, 具体操作请参考[集成步骤说明](./ysyx/soc/soc.md)
...
* 通过正常模式(不跳过SPI传输, 仿真速度慢, 用于最终的系统测试)对flash进行模拟,
  ...
  * [ ] memtest-loader.bin
  * [x] rtthread-loader.bin
* [x] 若为了正确运行测试程序而修改了设计, 需要重新进行代码规范检查,
      并更新记录Warning的表格文件中报告Warning的代码位置

### 提交

* 准备以下内容:
  * [x] 一份CPU的.v文件, 注意无需包含RAM的行为模型
  * [x] 记录无法清理的Verilator Warning的表格
