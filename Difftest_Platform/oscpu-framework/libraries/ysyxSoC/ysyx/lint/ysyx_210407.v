...
`define DEBUG
`define START_ADDR 64'h30000000
...
*/
    ...
    assign RamReadAddr     = ClintBus ? `ZERO_64      : BusReadAddr;
    assign RamReadEnable   = ClintBus ? 1'b0          : BusReadEnable;
    assign ClintReadAddr   = ClintBus ? BusReadAddr   : `ZERO_64;
    assign ClintReadEnable = ClintBus ? BusReadEnable : 1'b0;
    assign BusReadData     = ClintBus ? ClintReadData : RamReadData;

    assign RamWriteAddr     = ClintBus ? `ZERO_64       : BusWriteAddr;
    assign RamWriteData     = ClintBus ? `ZERO_64       : BusWriteData;
    assign RamWriteEnable   = ClintBus ? 1'b0           : BusWriteEnable;
    assign RamWriteMask     = ClintBus ? `ZERO_8       : BusWriteMask;
    assign ClintWriteAddr   = ClintBus ? BusWriteAddr   : `ZERO_64;
    assign ClintWriteData   = ClintBus ? BusWriteData   : `ZERO_64;
    ...
    wire [`DATA_BUS] BusReadData; 
    wire [`DATA_BUS] BusWriteData;
    wire [7 : 0] BusWriteMask;
    wire BusReadEnable;
    wire BusWriteEnable;

    wire [`ADDR_BUS] ClintReadAddr;
    wire [`ADDR_BUS] ClintWriteAddr;
    wire [`DATA_BUS] ClintReadData; 
    wire [`DATA_BUS] ClintWriteData;
...
module ysyx_210407_if_stage(input wire clk,
    ...
    assign pcF = rst ? `START_ADDR : jump ? jAddr : pc;
...
module ysyx_210407_ex_stage(input wire clk,
    ...
    // 使用pc还是寄存器
    wire  [`REG_BUS] adder1 = (instTypeE[6] | instTypeE[3] | instTypeE[5]) ? pcE : rs1Data;
    // 是否是Rtype
    ...
    wire [`REG_BUS] csrop2 = instTypeE[9] ? imm :
    instTypeE[8] ? rs1Data : `ZERO_64;
...
module ysyx_210407_me_stage(input rst,
    ...
    wire [`DATA_BUS]loadRes = BusReadDataM>>ReadShamt;
    ...
    /*
    wire [`DATA_BUS] loadRes8  = {56'b0 ,BusReadDataM[7 : 0]};
    wire [`DATA_BUS] loadRes16 = {48'b0 ,BusReadDataM[15 : 0]};
    wire [`DATA_BUS] loadRes32 = {32'b0 ,BusReadDataM[31 : 0]};
    wire [`DATA_BUS] loadRes64 = BusReadDataM[63 : 0];
    */
    

    wire [2 : 0] WriteShamt = BusWriteAddrM[2 : 0];
    
    wire [`DATA_BUS] storeRes8     = BusWriteDataM<<WriteShamt;
    wire [7 : 0] BusWriteMask8 = 8'b1<<WriteShamt;
    
    wire [`DATA_BUS] storeRes16     = BusWriteDataM<<WriteShamt;
    wire [7 : 0] BusWriteMask16 = 8'b11<<WriteShamt;
    
    wire [`DATA_BUS] storeRes32     = BusWriteDataM<<WriteShamt;
    wire [7 : 0] BusWriteMask32 = 8'b1111<<WriteShamt;
    
    wire [`DATA_BUS] storeRes64     = BusWriteDataM;
    wire [7 : 0] BusWriteMask64 = 8'b1111_1111;
    /*
    wire [`DATA_BUS] storeRes8     = BusWriteDataM;
    wire [`DATA_BUS] storeRes16     = BusWriteDataM;
    wire [`DATA_BUS] storeRes32     = BusWriteDataM;
    wire [`DATA_BUS] storeRes64    = BusWriteDataM;
    wire [`DATA_BUS] BusWriteMask8 = 64'hff;
    wire [`DATA_BUS] BusWriteMask16 = 64'hffff;
    wire [`DATA_BUS] BusWriteMask32 = 64'hffff_ffff;
    ...
    */
    ...
    memFuncM[3] ? BusWriteMask8  : `ZERO_8;
    ...
    assign BusWriteMask   = (rst == 1'b1) ? `ZERO_8 : BusWriteMask_r;
...
module ysyx_210407_csrfile(input wire clk,
     ...
     */
    wire [`REG_BUS] mvendorid = {32'b0, 32'h2000_0915};
    wire [`REG_BUS] marchid = {1'b1,63'b0};
    wire [`REG_BUS] mimpid = 64'b1;
    wire [`REG_BUS] mhartid = 64'b0;
    ...
    wire [`REG_BUS] misa = {2'b1,36'b0,26'b1_0000_0000};
    ...
    wire [`REG_BUS] ReadData_r = rst ? `ZERO_64 :                                  
                                 ReadCSR == `sstatus    ? sstatus     :
                                 ReadCSR == `mstatus    ? mstatus     :
                                 ReadCSR == `misa       ? misa        :
                                 ReadCSR == `medeleg    ? medeleg     :
                                 ReadCSR == `mideleg    ? mideleg     :
                                 ReadCSR == `mie        ? mie         :
                                 ReadCSR == `mtvec      ? mtvec       :
                                 ReadCSR == `mvendorid  ? mvendorid   :
                                 ReadCSR == `marchid    ? marchid     :
                                 ReadCSR == `mimpid     ? mimpid      :
                                 ReadCSR == `mhartid    ? mhartid     :
                                 ReadCSR == `mscratch   ? mscratch    :
                                 ReadCSR == `mepc       ? mepc        :
    ...
    */
    ...
    // mcycle
    wire [`REG_BUS]mcycle_t = rst ? 64'b0 : ((WriteCSR == `mcycle) & WriteEnable) ? WriteData : mcycle;
    wire [`REG_BUS]mcycle_r = rst ? 64'b0 : mcycle_t + 1;
    ysyx_210407_gen_en_dff #(64) mcycle_dff(mcycle_r,mcycle,1'b1&(~CoreLock),clk,rst);



    // minstret
    wire [`REG_BUS]minstret_t = rst ? 64'b0 : ((WriteCSR == `minstret) & WriteEnable) ? WriteData : minstret;
    wire [`REG_BUS]minstret_r = rst ? 64'b0 : (pcW != `ZERO_64) ? minstret_t + 1 : minstret_t; 
    ysyx_210407_gen_en_dff #(64) minstret_dff(minstret_r,minstret,1'b1&(~CoreLock),clk,rst);

    // medeleg
    wire [`REG_BUS]medeleg_t = rst ? 64'b0 : ((WriteCSR == `medeleg) & WriteEnable) ? WriteData : medeleg;
    wire [`REG_BUS]medeleg_r = rst ? 64'b0 : (IRQtype[1]) ? medeleg_t + 1 : medeleg_t; 
    ysyx_210407_gen_en_dff #(64) medeleg_dff(medeleg_r,medeleg,1'b1&(~CoreLock),clk,rst);

    // mideleg
    wire [`REG_BUS]mideleg_t = rst ? 64'b0 : ((WriteCSR == `mideleg) & WriteEnable) ? WriteData : mideleg;
    wire [`REG_BUS]mideleg_r = rst ? 64'b0 : (IRQtype[0]) ? mideleg_t + 1 : mideleg_t; 
    ...
    ((ClintReadAddr == 64'h2004000) & ClintReadEnable) ? mtimecmp : `ZERO_64;
